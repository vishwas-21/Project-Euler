games = ["8C TS KC 9H 4S 7D 2S 5D 3S AC\
...
def get_one_pair(hand):
    values = [card[0] for card in hand]
    for el in values:
        if values.count(el) == 2:
            return card_values[el]
...
def get_two_pairs(hand):
    ...
    for el in values:
        if values.count(el) == 2:
            pairs.append(card_values[el])
    return max(pairs)

def is_three_of_a_kind(hand):
    values = [card[0] for card in hand]
    for el in values:
        if values.count(el) == 3:
            return True
    return False
...
def is_straight(hand):
    values = [card[0] for card in hand]
    values.sort(key=lambda x: card_values[x])
    for i in range(len(values) - 1):
        if card_values[values[i]] != card_values[values[i + 1]] - 1:
            return False
    return True
...
def high_card(hand_a, hand_b):
    values_a = [card_values[card[0]] for card in hand_a]
    values_b = [card_values[card[0]] for card in hand_b]
    values_a.sort(reverse=True)
...
games = [el for el in games[0].replace(' ', '', -1)]
games = [games[i:i+20] for i in range(0, len(games), 20)]
games = [[game[i:i+2] for i in range(0, len(game), 2)] for game in games]
...
for game in games:
    ...
    if rank_a > rank_b:
        ...
        if rank_a in [10, 9, 6, 5, 1]:
            ans += high_card(hand_a, hand_b)

        elif rank_a == 2:
            a, b = get_one_pair(hand_a), get_one_pair(hand_b)
            if a > b:
                ans += 1
            elif a == b:
                ans += high_card(hand_a, hand_b)
            ...
            a, b = get_two_pairs(hand_a), get_two_pairs(hand_b)
            if a > b:
                ans += 1
            elif a == b:
                ans += high_card(hand_a, hand_b)
            
        elif rank_a == 4 or rank_a == 7:
            a, b = get_three_of_a_kind(hand_a), get_three_of_a_kind(hand_b)
            if a > b:
                ans += 1
            elif a == b:
                ans += high_card(hand_a, hand_b)
            ...
            a, b = get_four_of_a_kind(hand_a), get_four_of_a_kind(hand_b)
            if a > b:
                ans += 1
            elif a == b:
                ans += high_card(hand_a, hand_b)
  
print(f"Player 1 won {ans} games!")